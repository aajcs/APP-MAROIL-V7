...
import ReporteCargaGOMInfoCardEndeco from './ReporteCargaGOMInfoCardEndeco'
import ProgramacionVentanaEndeco from './ProgramacionVentanaEndeco'

function ReporteCargaGOMInfoEndeco() {
      ...
      {barcos.length === 0 ? (
        ...
        barcos.map(
          (barcos) =>
            ...
            barcos.buqueClienteVenta === 'ENDECO' &&
            barcos.reporteCargaGOM.length !== 0 && (
              <ReporteCargaGOMInfoCardEndeco key={barcos.id} barcos={barcos} />
            )
        )
      )}
      <ProgramacionVentanaEndeco></ProgramacionVentanaEndeco>
    </>
  )
}

export default ReporteCargaGOMInfoEndeco
